...
import com.hooha.maidai.chat.presenters.viewinface.MembersDialogView;
...
public class LiveHelper extends Presenter {
    ...
    private static final int MAX_REQUEST_VIEW_COUNT = 2; //当前最大支持请求画面个数
        ...
        try {
            ...
            switch (action) {
                ...
                case Constant.AVIMCMD_Praise:
                    mLiveView.refreshThumbUp(nickname);
                    break;
                case Constant.AVIMCMD_Interaction_Praise:
                    mLiveView.refreshThumbUp2(nickname);
                    break;
                case Constant.AVIMCMD_Host_Flower:
                    Log.d(TAG, "refreshFlower: AVIMCMD_Host_Flower");
                    mLiveView.refreshFlower(nickname);
                    break;
                case Constant.AVIMCMD_Interaction_Flower:
                    mLiveView.refreshFlower2(nickname);
                    break;
                case Constant.AVIMCMD_Host_Present:
                    mLiveView.refreshGratuity(nickname);
                    break;
                case Constant.AVIMCMD_Interaction_Present:
                    mLiveView.refreshGratuity2(nickname);
                    break;
                case Constant.AVIMCMD_Ready_Game:
                    mLiveView.readyGame();
                    break;
                case Constant.AVIMCMD_Start_Game:
                    mLiveView.startGame();
                    break;
                    ...
                    public void OnComplete(int arg0) {
                        ...
                        if (arg0 == AVError.AV_OK) {
                            if (leverupper == true) {
                                ...
                                sendC2CMessage(Constant.AVIMCMD_MUlTI_JOIN, "", CurLiveInfo.getHostID());//发送回应消息
                            } else {
                                closeCameraAndMic();
                            }
