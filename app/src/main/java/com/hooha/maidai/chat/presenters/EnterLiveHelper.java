...
//                mStepInOutView.enterRoomComplete(MySelfInfo.getInstance().getIdStatus(), false);
        ...
        public void onEndpointsUpdateInfo(int eventid, String[] updateList) {
            ...
            switch (eventid) {
                ...
                case TYPE_MEMBER_CHANGE_NO_CAMERA_VIDEO: {
                    ...
                    Intent closeintent = new Intent(Constant.ACTION_CAMERA_CLOSE_IN_LIVE);
     ...
     * 1_5上传房间信息
     */
    public void notifyServerCreateRoom(final int type) {
...
//                    liveInfo.put("avRoomId", CurLiveInfo.getRoomNum());
                    liveInfo.put("avRoomId", MySelfInfo.getInstance().getMyRoomNum());
                    liveInfo.put("timeSpan", 0);
                    liveInfo.put("watchCount", 0);
                    liveInfo.put("admireCount", 0);
                    liveInfo.put("totalGratuity", 0);
                    liveInfo.put("totalFlower", 0);
                    liveInfo.put("activeId", "" + type);
    ...
    private void joinIMChatRoom(int chatRoomId) {
        ...
        TIMGroupManager.getInstance().applyJoinGroup("" + chatRoomId, Constant.APPLY_CHATROOM + chatRoomId, new TIMCallBack() {
            @Override
            public void onError(int i, String s) {
                //已经在是成员了
...
//            notifyServerLiveEnd();
    ...
    private void EnterAVRoom(int roomNum) {
        ...
        if (MySelfInfo.getInstance().getIdStatus() == Constant.HOST) {
            enterRoomParam.authBits = Constant.HOST_AUTH;//；TODO：主播权限 所有权限
            enterRoomParam.avControlRole = Constant.HOST_ROLE;//；TODO：主播角色
            enterRoomParam.autoCreateRoom = true;//;TODO：主播自动创建房间
        } else {
//            enterRoomParam.authBits = Constant.NORMAL_MEMBER_AUTH;//；TODO：普通成员权限
            enterRoomParam.authBits = Constant.HOST_AUTH;//；TODO：主播权限
            enterRoomParam.avControlRole = Constant.NORMAL_MEMBER_ROLE;//；TODO：普通成员角色
        ...
        enterRoomParam.audioCategory = Constant.AUDIO_VOICE_CHAT_MODE;//；TODO：音频场景策略
